...
function loadConfig(): AuthConfiguration {
  ...
  return {
    jwt: {
      ...
      rotationSchedule:
        process.env.JWT_ROTATION_SCHEDULE || JWT_DEFAULTS.rotationSchedule,
    },
    oauth2: {
      ...
      authorizeUrl:
        process.env.OAUTH_AUTHORIZE_URL || "https://auth0.com/authorize",
      tokenUrl: process.env.OAUTH_TOKEN_URL || "https://auth0.com/oauth/token",
      userInfoUrl:
        process.env.OAUTH_USER_INFO_URL || "https://auth0.com/userinfo",
    ...
    session: {
      ...
      maxAge: parseInt(
        process.env.SESSION_MAX_AGE || SESSION_DEFAULTS.maxAge.toString(),
        10
      ),
    ...
    security: {
      maxLoginAttempts: parseInt(
        process.env.MAX_LOGIN_ATTEMPTS ||
          SECURITY_DEFAULTS.maxLoginAttempts.toString(),
        10
      ),
      lockoutDuration: parseInt(
        process.env.LOCKOUT_DURATION ||
          SECURITY_DEFAULTS.lockoutDuration.toString(),
        10
      ),
      passwordPolicy: {
        minLength: parseInt(
          process.env.PASSWORD_MIN_LENGTH ||
            SECURITY_DEFAULTS.passwordPolicy.minLength.toString(),
          10
        ),
        ...
        maxAge: parseInt(
          process.env.PASSWORD_MAX_AGE ||
            SECURITY_DEFAULTS.passwordPolicy.maxAge.toString(),
          10
        ),
        preventReuse: parseInt(
          process.env.PASSWORD_PREVENT_REUSE ||
            SECURITY_DEFAULTS.passwordPolicy.preventReuse.toString(),
          10
        ),
      },
      mfa: {
        ...
        allowedTypes: (
          process.env.MFA_ALLOWED_TYPES ||
          SECURITY_DEFAULTS.mfa.allowedTypes.join(",")
        ).split(","),
        issuer: process.env.MFA_ISSUER || "AUSTA SuperApp",
        validityWindow: parseInt(
          process.env.MFA_VALIDITY_WINDOW ||
            SECURITY_DEFAULTS.mfa.validityWindow.toString(),
          10
        ),
        backupCodes: parseInt(
          process.env.MFA_BACKUP_CODES ||
            SECURITY_DEFAULTS.mfa.backupCodes.toString(),
          10
        ),
      },
      hipaa: {
        ...
        inactivityTimeout: parseInt(
          process.env.HIPAA_INACTIVITY_TIMEOUT ||
            HIPAA_DEFAULTS.inactivityTimeout.toString(),
          10
        ),
        sessionMonitoring: process.env.HIPAA_SESSION_MONITORING !== "false",
        encryptionStrength:
          process.env.HIPAA_ENCRYPTION_STRENGTH ||
          HIPAA_DEFAULTS.encryptionStrength,
      },
    },
    redis: {
      cluster: {
        ...
        options: {
          maxRedirections: parseInt(
            process.env.REDIS_MAX_REDIRECTIONS || "16",
            10
          ),
          retryDelayOnFailover: parseInt(
            process.env.REDIS_RETRY_DELAY_FAILOVER || "3000",
            10
          ),
          retryDelayOnClusterDown: parseInt(
            process.env.REDIS_RETRY_DELAY_DOWN || "1000",
            10
          ),
